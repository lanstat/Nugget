...
class monitor:
	...
	def __pppd_monitor(self):
		...
		if self.pppd_pid == None :
			...
			if pppd_pid != "" :
				...
				self.current_state = "Buscando IP..."
			...
			if out != "" :
				...
				self.current_state = "Coneccion establecida :D"
				self.__set_dns_info()
	...
	def __set_dns_info(self):
		...
		os.system("echo ';Annihilator manager dns data' > /etc/resolv.conf")
		if self.dns_data[2] != "" :
